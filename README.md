...
Интерфейс на арабском (RTL, по умолчанию) и французском (LTR). Основной CTA ведёт на сайт
рекламодателя и переносит в него UTM-метки. Аналитика шлёт `page_view` и `cta_click`.
...
```js
ctaUrl: 'https://melbet.com',            // адрес оффера
...
`https://melbet.com`, то есть на сайт рекламодателя.

В живой связке на этом месте стоит партнёрская трекинговая ссылка с subid, которая уже сама
редиректит на оператора. Такую ссылку в задании не выдавали, поэтому CTA ведёт на основной домен
бренда напрямую: механика подстановки и переноса меток от этого не меняется и её видно в деле.
Адрес живёт одной строкой в `assets/config.js`, обе кнопки и сборка берут значение оттуда.
Если у команды используется гео-домен для Алжира, например `melbet-dz.com`, меняется та же строка.
...
| UTM доезжают до CTA | `https://melbet.com/?utm_source=fb&utm_campaign=dz_mlb&subid=abc` |
